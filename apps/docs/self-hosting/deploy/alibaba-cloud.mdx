...
<Note>
  Additionally, Alibaba Cloud offers a streamlined setup process using Docker
  Compose for quickly launching the entire Typebot application. It automatically
  creates OAuth credentials for Alibaba Cloud and configures a public IP for
  your application. Throughout the deployment process, you'll only need to
  provide a few simple parameters to start using Typebot seamlessly.
  Furthermore, if you already have a GitHub OAuth Client created, the deployment
  process also supports enabling GitHub OAuth login directly.
</Note>
## Requirements
